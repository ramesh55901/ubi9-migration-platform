...
def find_dockerfiles(repo: Path) -> list[Path]:
    ...
    for p in repo.rglob("*"):
        ...
        if (
            p.is_file()
            and (p.name == "Dockerfile" or p.name.startswith("Dockerfile."))
            and not p.name.endswith(".ubi9")
        ):
            files.append(p)
    return sorted(files)
...
def analyze(repo: str | Path, source: str | None = None) -> dict:
    ...
    selected = (
        (repo / source).resolve()
        if source
        else (dockerfiles[0] if len(dockerfiles) == 1 else None)
    )
    text = selected.read_text(errors="replace") if selected else ""
    from_images = re.findall(
        r"^FROM\s+(?:--platform=\S+\s+)?(\S+)", text, flags=re.MULTILINE | re.IGNORECASE
    )
    managers = []
    for token in ("apt-get", "apt ", "apk ", "yum ", "dnf ", "microdnf"):
        if token in text:
            managers.append(token.strip())
    languages = []
    indicators = {
